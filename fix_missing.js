...
async function main() {
  const content = fs.readFileSync('pokedex_1025.ts', 'utf8');
  const jsonStr = content.replace('export const POKEDEX_BASE: PokemonBase[] = ', '').replace(/;$/, '');
  const pokemonList = JSON.parse(jsonStr);
